...
function bukaUrl(){
  var x=document.getElementsByTagName("a");
  for(var i=0;i<x.length;i++){ 
    if(x[i].href.indexOf(arguments[0])!=-1&&x[i].innerText===arguments[1]){
      buka(x[i].href);
      break;
    }
  }
...
/*end*/
  ...
  if(document.URL.indexOf("mbasic/more/?owner_id=")!=-1){ 

bukaUrl("removefriend.php?friend_id=","Batalkan pertemanan");
  } 
if(document.URL.indexOf("removefriend.php?friend_id")!=-1&&document.URL.indexOf("unref=profile_gear")!=-1){
 setInterval(hapusTeman,2000);
  } 
